...
function deriveAction(method: string, path: string): string {
  ...
  if (method === "POST") {
    ...
    if (lowerPath.includes("/ban")) return "BAN";
    if (lowerPath.includes("/unban")) return "UNBAN";
    if (lowerPath.includes("/refund")) return "REFUND";
    if (lowerPath.includes("/broadcast")) return "BROADCAST";
    if (lowerPath.includes("/impersonate")) return "IMPERSONATE";
    if (lowerPath.includes("/assign-instructor")) return "UPDATE";
    if (lowerPath.includes("/feature")) return "UPDATE";
    if (lowerPath.includes("/link")) return "UPDATE";
    if (lowerPath.includes("/attempt")) return "QUIZ_ATTEMPT";
    if (lowerPath.includes("/submit") && lowerPath.includes("/quizzes")) return "QUIZ_SUBMIT";
    if (lowerPath.includes("/submit") && lowerPath.includes("/assignments")) return "ASSIGNMENT_SUBMIT";
    if (lowerPath.includes("/unsubmit") && lowerPath.includes("/assignments")) return "ASSIGNMENT_UNSUBMIT";
    if (lowerPath.includes("/complete") && lowerPath.includes("/lessons")) return "LESSON_COMPLETE";
    if (lowerPath.includes("/grade")) return "GRADE";
    if (lowerPath.includes("/submit-review")) return "SUBMIT_REVIEW";
    if (lowerPath.includes("/enrollments")) return "ENROLL";
    if (lowerPath.includes("/discussions")) return "DISCUSSION_POST";
    if (lowerPath.includes("/replies")) return "DISCUSSION_REPLY";
    return "CREATE";
  }
 ...
 */
function deriveResource(path: string): { resource: string | null; resourceId: string | null } {
  // Strip query string
...
function buildDescription(action: string, resource: string | null, resourceId: string | null): string {
...
export function auditLog(req: Request, res: Response, next: NextFunction) {
  ...
  res.on("finish", async () => {
    try {
      ...
      if (!actor) return; // nothing to attribute to
 ...
 */
export function errorLog(err: any, req: Request, res: Response, next: NextFunction) {
  if (req.user) {
