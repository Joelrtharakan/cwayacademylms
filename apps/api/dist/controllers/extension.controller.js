...
const getExtensionRequests = async (req, res) => {
    ...
    try {
        const requests = await prisma_1.prisma.extensionRequest.findMany({
            where: { courseId },
            include: {
...
exports.getExtensionRequests = getExtensionRequests;
const getStudentExtensionRequests = async (req, res) => {
    const { courseId } = req.params;
    ...
    try {
        ...
        const granted = await prisma_1.prisma.extension.findMany({
            where: { courseId, studentId },
        });
...
exports.getStudentExtensionRequests = getStudentExtensionRequests;
const createExtensionRequest = async (req, res) => {
    ...
    try {
        ...
        const existing = await prisma_1.prisma.extensionRequest.findFirst({
        ...
        const request = await prisma_1.prisma.extensionRequest.create({
            data: {
                studentId,
                courseId,
                itemId,
...
const updateExtensionRequestStatus = async (req, res) => {
    ...
    try {
        const request = await prisma_1.prisma.extensionRequest.findUnique({ where: { id } });
        if (!request)
            return res.status(404).json({ success: false, message: "Request not found" });
        // Update request status
        const updated = await prisma_1.prisma.extensionRequest.update({
            where: { id },
            data: { status }
        });
        // If approved, create or update the actual Extension record
        if (status === "APPROVED" && extendedDate) {
            await prisma_1.prisma.extension.upsert({
                where: {
                    studentId_itemId: {
...
exports.updateExtensionRequestStatus = updateExtensionRequestStatus;
const createManualExtension = async (req, res) => {
    const { courseId } = req.params;
    const { studentId, itemId, itemType, extendedDate } = req.body;
    try {
        const extension = await prisma_1.prisma.extension.upsert({
            where: {
                studentId_itemId: {
                    studentId,
                    itemId,
...
exports.createManualExtension = createManualExtension;
