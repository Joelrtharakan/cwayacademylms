...
exports.CertificateService = exports.PROGRAM_CERTIFICATE_HTML = exports.COURSE_CERTIFICATE_HTML = void 0;
...
`;
// ─── Shared frame HTML ───────────────────────────────────────────────────────
function frameWrapper(innerContent) {
    return `<!DOCTYPE html>
<html>
<head>
  <meta charset="UTF-8">
...
exports.COURSE_CERTIFICATE_HTML = frameWrapper(`
...
exports.PROGRAM_CERTIFICATE_HTML = frameWrapper(`
      <div class="top-section">
        <!-- HEADER -->
        <div class="org-title"><span class="cway">CWAY</span> <span class="academy">ACADEMY</span></div>
        <div class="org-subtitle">Coach, Challenge, and Commission</div>
...
class CertificateService {
    ...
    static async issueCertificate(studentId, courseId) {
        ...
        try {
            const { sendCertificateIssuedEmail } = await Promise.resolve().then(() => __importStar(require('./email.service')));
            await sendCertificateIssuedEmail({ name: certificate.student.name, email: certificate.student.email }, { title: certificate.course?.title || 'Course' }, certificate.uniqueCode);
        }
        catch (e) {
    ...
    static async issueProgramCertificate(studentId, programId) {
        const existing = await prisma_1.prisma.certificate.findFirst({
        ...
        try {
            const { sendCertificateIssuedEmail } = await Promise.resolve().then(() => __importStar(require('./email.service')));
            await sendCertificateIssuedEmail({ name: certificate.student.name, email: certificate.student.email }, { title: certificate.program?.title || 'Program' }, certificate.uniqueCode);
        }
        catch (e) {
            console.error('[Email] Failed to send certificate issued email:', e);
        }
