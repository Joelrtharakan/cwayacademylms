...
exports.sendEnrollmentConfirmationEmail = sendEnrollmentConfirmationEmail;
...
async function sendVerificationEmail(user, token) {
    ...
    await send(user.email, 'Verify your CWAY Academy account', wrap(`
...
async function sendWelcomeEmail(user) {
    await send(user.email, 'Welcome to CWAY Academy!', wrap(`
...
async function sendPasswordResetEmail(user, token) {
    ...
    await send(user.email, 'Reset your CWAY Academy password', wrap(`
...
async function sendEnrollmentConfirmationEmail(user, course) {
    await send(user.email, `You are enrolled in "${course.title}"!`, wrap(`
...
async function sendCertificateIssuedEmail(user, course, uniqueCode) {
    await send(user.email, `Your certificate is ready — "${course.title}"`, wrap(`
...
async function sendCourseApprovedEmail(instructor, course) {
    await send(instructor.email, `Your course has been approved — "${course.title}"`, wrap(`
...
async function sendSponsorThankYouEmail(sponsor, amount) {
    await send(sponsor.email, `Thank you for your generosity, ${sponsor.name}!`, wrap(`
...
async function sendInstructorWelcomeEmail(instructor, password) {
    await send(instructor.email, `Welcome to CWAY Academy as an Instructor!`, wrap(`
