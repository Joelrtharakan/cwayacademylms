...
import { getProgramStudentDetails } from "@/lib/api/registrar";
...
export default function ProgramStudentDetailsPage() {
  ...
  if (error || !data) {
    return (
      <div style={{ textAlign: "center", padding: "80px 40px" }}>
        ...
        <Link href={`/registrar/programs/${id}`} style={{ color: "#C9973A" }}>← Back to Program</Link>
      </div>
    );
  }
  ...
  const handleViewCertificate = async (id: string) => {
    ...
    try {
      ...
      `);
      const res = await api.get(`/registrar/certificates/${id}/download`, { responseType: 'blob' });
