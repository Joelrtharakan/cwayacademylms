...
import { getUsers, banUser, unbanUser, deleteUser, impersonateUser, exportUsersCSV } from "@/lib/api/registrar";
import * as DropdownMenu from "@radix-ui/react-dropdown-menu";

const ROLE_COLORS: Record<string, { bg: string; color: string }> = {
  ADMIN:      { bg: "rgba(60,52,137,0.08)", color: "#3c3489" },
  REGISTRAR:  { bg: "rgba(79,70,229,0.1)", color: "#4F46E5" },
  ...
  const actions = (row: any) => (
    <DropdownMenu.Root>
      ...
      <DropdownMenu.Portal>
        ...
        >
          {[
            { label: "View Profile", icon: <Eye size={14} />, onClick: () => router.push(`/registrar/users/${row.id}`) },
            {
        ...
        {[
          {
            ...
            options: [["", "All Roles"], ["ADMIN", "Registrar"], ["INSTRUCTOR", "Instructor"], ["STUDENT", "Student"]],
          },
          {
            value: statusFilter,
