...
export default function NewAdminCoursePage() {
  ...
  const { data: catData } = useQuery({
    queryKey: ["categories"],
    queryFn: () => api.get("/registrar/categories").then((r) => r.data.data),
  });

  const { data: instData } = useQuery({
    queryKey: ["instructors"],
    queryFn: () => api.get("/registrar/instructors").then((r) => r.data.data),
  });

  const createMut = useMutation({
    mutationFn: (data: any) => api.post("/registrar/courses", data).then((r) => r.data.data),
    onSuccess: (data) => {
      toast.success("Course created & instructor invited");
      router.push(`/registrar/courses`);
    },
    onError: (e: any) => {
    ...
    <div style={{ minHeight: "100vh", background: "#F7F8F5", display: "flex", alignItems: "center", justifyContent: "center", padding: "40px 20px" }}>
      ...
      >
        ...
        <div style={{ padding: "32px 40px", borderBottom: "1px solid rgba(184,134,69,0.15)", position: "relative" }}>
          <Link href="/registrar/courses" style={{ position: "absolute", left: "40px", top: "38px", color: "#8F9E93", transition: "color 0.2s" }} onMouseEnter={(e) => e.currentTarget.style.color = "#F5F0E8"} onMouseLeave={(e) => e.currentTarget.style.color = "#8A9E8C"}>
            <ArrowLeft size={20} />
          </Link>
        ...
        <div style={{ padding: "40px" }}>
          <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "32px" }}>
            ...
            <div style={{ paddingTop: "16px", borderTop: "1px solid rgba(184,134,69,0.15)", display: "flex", justifyContent: "flex-end", gap: "16px" }}>
              <Link href="/registrar/courses" style={{ padding: "14px 28px", borderRadius: "999px", fontSize: "15px", fontWeight: 600, color: "#8F9E93", textDecoration: "none", display: "flex", alignItems: "center" }}>
                Cancel
              </Link>
              <button
                type="submit"
