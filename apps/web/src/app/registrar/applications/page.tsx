...
export default function AdminApplicationsPage() {
  const { data: applications, isLoading } = useQuery({
    ...
    queryFn: () => api.get("/registrar/applications").then(res => res.data.data),
  });
              ...
              {applications?.map((app: any) => (
                <tr key={app.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="whitespace-nowrap text-right text-sm font-medium" style={{ padding: "20px 24px" }}>
                    <Link href={`/registrar/applications/${app.id}`} className="inline-flex items-center justify-center bg-white border border-gray-300 rounded-lg text-sm font-semibold text-gray-700 hover:bg-gray-50 transition-all shadow-sm" style={{ padding: "8px 20px" }}>
                      Review
                    </Link>
                  </td>
                </tr>
