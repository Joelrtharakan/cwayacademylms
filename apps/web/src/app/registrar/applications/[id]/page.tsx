...
export default function ApplicationDetailsPage() {
  ...
  const { data: app, isLoading } = useQuery({
    ...
    queryFn: () => api.get(`/registrar/applications/${id}`).then(res => res.data.data),
  });

  const approveMutation = useMutation({
    mutationFn: () => api.post(`/registrar/applications/${id}/approve`),
    onSuccess: () => {
  ...
  const rejectMutation = useMutation({
    mutationFn: () => api.post(`/registrar/applications/${id}/reject`),
    onSuccess: () => {
    ...
    <div className="p-8">
      ...
      <div className="print:hidden flex justify-between items-center" style={{ marginBottom: "48px" }}>
        <div className="flex items-center gap-4">
          <Link href="/registrar/applications" className="text-gray-500 hover:text-gray-900">
            <ArrowLeft size={20} />
          </Link>
          <div>
