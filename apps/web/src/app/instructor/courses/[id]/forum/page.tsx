...
export default function CourseForumPage() {
  ...
  return (
    <div style={{ display: "flex", height: "calc(100vh - 60px - 48px)", gap: 0, background: SURFACE, borderRadius: 16, overflow: "hidden", border: "1px solid rgba(184,134,69,0.2)" }}>
      {/* Left: Posts List */}
      <div style={{ width: 340, flexShrink: 0, borderRight: "1px solid rgba(255,255,255,0.06)", display: "flex", flexDirection: "column" }}>
        <div style={{ padding: "16px 20px 12px", display: "flex", alignItems: "center", gap: 12, borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
          <button onClick={() => router.back()} style={{ background: "#F7F8F5", border: "none", borderRadius: "50%", width: 32, height: 32, display: "flex", alignItems: "center", justifyContent: "center", color: "#1A261D", cursor: "pointer" }}>
            <ArrowLeft size={16} />
          </button>
          <div>
            <h2 style={{ fontFamily: "var(--font-dm-serif), serif", fontSize: 18, color: "#1A261D", margin: 0 }}>Course Forum</h2>
          ...
          {posts.length === 0 ? (
            ...
            posts.map((p: any) => (
              <button key={p.id} onClick={() => setSelectedPostId(p.id)}
                style={{ width: "100%", textAlign: "left", padding: "16px 20px", background: selectedPostId === p.id ? "rgba(184,134,69,0.1)" : "transparent", border: "none", borderBottom: "1px solid rgba(255,255,255,0.04)", cursor: "pointer", transition: "all 0.15s" }}>
      ...
      {selectedPost ? (
        <div style={{ flex: 1, display: "flex", flexDirection: "column" }}>
          <div style={{ flex: 1, overflowY: "auto", padding: 32 }}>
            ...
            <div style={{ marginBottom: 32 }}>
              <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 16 }}>
                <div>
                  <h1 style={{ fontFamily: "var(--font-dm-serif), serif", fontSize: 24, color: "#1A261D", marginBottom: 8 }}>{selectedPost.title}</h1>
              ...
              <div style={{ fontSize: 14, color: "#1A261D", lineHeight: 1.6, background: "#F7F8F5", padding: 20, borderRadius: 12, border: "1px solid #F7F8F5" }}>
                {selectedPost.content}
              </div>
            </div>

            {/* Replies */}
            <div style={{ paddingLeft: 24, borderLeft: "2px solid #F7F8F5" }}>
              {selectedPost.replies?.map((r: any) => (
                <div key={r.id} style={{ marginBottom: 20, position: "relative" }}>
                  <div style={{ position: "absolute", left: -34, top: 12, color: "#E4E8E0" }}><CornerDownRight size={16} /></div>
                  <div style={{ background: r.author.role === "INSTRUCTOR" ? "rgba(184,134,69,0.06)" : "#F7F8F5", border: `1px solid ${r.author.role === "INSTRUCTOR" ? "rgba(184,134,69,0.2)" : "#F7F8F5"}`, borderRadius: 12, padding: "16px 20px" }}>
                    ...
                    <div style={{ fontSize: 13, color: "#1A261D", lineHeight: 1.5 }}>{r.content}</div>
                  </div>
                </div>
              ))}
            </div>
          ...
          {/* Reply Box */}
          <div style={{ padding: "16px 32px", borderTop: "1px solid rgba(255,255,255,0.06)", background: "#F7F8F5" }}>
            <div style={{ display: "flex", gap: 12 }}>
              <textarea value={replyContent} onChange={e => setReplyContent(e.target.value)} placeholder="Type your reply..." rows={2}
                style={{ flex: 1, background: "rgba(255,255,255,0.04)", border: "1px solid rgba(184,134,69,0.2)", borderRadius: 10, padding: "12px 16px", color: "#1A261D", fontSize: 13, outline: "none", resize: "vertical", fontFamily: "inherit" }} />
