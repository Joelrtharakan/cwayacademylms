...
export default function CourseStudentsPage() {
  ...
  const { data: studentsData } = useQuery({
    ...
    queryFn: () => api.get(`/instructor/courses/${id}/students`).then(r => r.data.data),
  });
  ...
  const handleExportCSV = () => {
    ...
    const rows = filtered.map((e: any) => [
      ...
      `"${e.lastCompleted?.title || 'Not started'}"`,
    ...
    const safeTitle = course?.title ? course.title.replace(/[^\w\s-]/g, '').trim() : `course_${id}`;
    link.setAttribute("download", `${safeTitle} - Enrolled Students.csv`);
    ...
    <div style={{ padding: "24px 32px", maxWidth: 1000 }}>
      ...
      <div style={{ background: SURFACE, border: "1px solid var(--border-light, #E2E8F0)", borderRadius: 16, overflow: "hidden", boxShadow: "var(--shadow-sm, 0 1px 3px rgba(0,0,0,0.05))" }}>
        <div style={{ padding: "20px 24px", borderBottom: "1px solid var(--border-light, #E2E8F0)", display: "flex", justifyContent: "space-between", alignItems: "center", background: "#FFFFFF" }}>
          ...
          <button onClick={handleExportCSV} style={{ display: "flex", alignItems: "center", gap: 8, background: "#FFFFFF", border: "1px solid var(--border-light, #E2E8F0)", color: DARK, borderRadius: 8, padding: "10px 20px", fontSize: 13, fontWeight: 600, cursor: "pointer", transition: "all 0.2s", boxShadow: "var(--shadow-sm, 0 1px 2px rgba(0,0,0,0.05))" }}>
        ...
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          <thead>
            <tr style={{ background: "var(--cream-mid, #F4F5F1)" }}>
              ...
              <th style={{ padding: "14px 24px", textAlign: "left", fontSize: 12, fontWeight: 700, color: MUTED, textTransform: "uppercase", letterSpacing: "0.08em" }}>Last Completed</th>
          ...
          <tbody>
            {filtered.map((e: any) => (
              <tr key={e.id} style={{ borderTop: "1px solid var(--border-light, #E2E8F0)", background: "#FFFFFF", transition: "background 0.15s" }}>
                ...
                <td style={{ padding: "16px 24px", fontSize: 13 }}>
                  {e.lastCompleted ? (
                    <div>
                      <div style={{ fontWeight: 600, color: DARK, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", maxWidth: 180 }} title={e.lastCompleted.title}>{e.lastCompleted.title}</div>
                      <div style={{ color: MUTED, fontSize: 12, marginTop: 2 }}>{format(new Date(e.lastCompleted.date), "MMM d, yyyy")}</div>
                    </div>
                  ) : (
                    <span style={{ color: MUTED, fontStyle: "italic" }}>Not started</span>
                  )}
                </td>
            ...
            {filtered.length === 0 && (
              <tr><td colSpan={5} style={{ padding: "48px", textAlign: "center", color: MUTED, fontSize: 14, background: "#FFFFFF" }}>No students found</td></tr>
            )}
          </tbody>
        </table>
      </div>
