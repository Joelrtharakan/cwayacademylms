  ...
  return (
    <div style={{ maxWidth: 1000, margin: "0 auto" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 16, marginBottom: 24 }}>
        ...
        <div>
          <h1 style={{ fontFamily: "var(--font-dm-serif), serif", fontSize: 24, color: "#1A261D", margin: 0 }}>Course Analytics</h1>
            ...
            <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 16 }}>
              {progressData.map(d => (
                <div key={d.name}>
                  ...
                  <div style={{ fontSize: 20, fontWeight: 700, color: "#1A261D", marginTop: 4, marginLeft: 18 }}>{d.value}</div>
                </div>
              ))}
            </div>
          </div>
      ...
      <div style={{ background: SURFACE, border: "1px solid rgba(184,134,69,0.2)", borderRadius: 12, padding: 24 }}>
        ...
        <ResponsiveContainer width="100%" height={260}>
          <BarChart data={analytics?.lessonCompletionRates} margin={{ left: -20 }} layout="vertical">
            ...
            <Tooltip contentStyle={{ background: "#FFFFFF", border: "1px solid rgba(184,134,69,0.3)", borderRadius: 8 }} labelStyle={{ color: "#1A261D" }} formatter={(v: any) => [`${Math.round(v)}%`, "Completed"]} />
