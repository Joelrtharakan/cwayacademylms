...
export default function CoursePlayerLayout({ children }: { children: React.ReactNode }) {
  ...
  const tPortal = useTranslations("student.sidebar");
            ...
            <div style={{ marginTop: "16px", background: "rgba(255,255,255,0.03)", borderRadius: "8px", border: "1px solid rgba(255,255,255,0.05)", overflow: "hidden" }}>
              ...
              >
                <div style={{ display: "flex", flexDirection: "column" }}>
                  <span style={{ fontSize: "11px", fontWeight: 600, color: "rgba(255,255,255,0.5)", textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: "2px" }}>{t("totalGrade")}</span>
                  <span style={{ fontSize: "10px", color: "rgba(255,255,255,0.3)" }}>{courseGrade.totalEarned} / {courseGrade.totalMaxGraded} {t("pts")}</span>
                </div>
              ...
              {isGradeExpanded && courseGrade.items && courseGrade.items.length > 0 && (
                <div style={{ borderTop: "1px solid rgba(255,255,255,0.05)", padding: "10px 12px", background: "rgba(0,0,0,0.15)" }}>
                  <div style={{ fontSize: "9px", fontWeight: 700, color: "rgba(255,255,255,0.4)", textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: "8px" }}>{t("gradeBreakdown")}</div>
                ...
                >
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: "12px", color: "rgba(255,255,255,0.4)" }}>
                      {t("items", { completed: mod.completedItems, total: mod.totalItems })}
                    </div>
                  </div>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0, position: "relative", overflowX: "hidden" }}>
        ...
        >
          <div style={{ padding: "20px 24px", borderBottom: "1px solid #E4E8E0", display: "flex", justifyContent: "space-between", alignItems: "center", background: "#FAFAF7" }}>
            <h2 style={{ fontFamily: "var(--font-dm-serif), serif", fontSize: "20px", color: "#1A261D", margin: 0 }}>{t("myNotes")}</h2>
            <button 
          ...
          <div style={{ flex: 1, padding: "24px", overflowY: "auto", background: "#FAFAF7" }}>
            {myNotes.length === 0 ? (
              <div style={{ fontSize: "13px", color: "#8F9E93", textAlign: "center", marginTop: "40px" }}>{t("noNotes")}</div>
            ) : (
