...
const NAV_KEYS = [
  {
    titleKey: "overview",
    items: [{ nameKey: "dashboard", href: "/student/dashboard", icon: LayoutDashboard }],
  },
  {
    titleKey: "learning",
    items: [
      { nameKey: "myCourses", href: "/student/my-courses", icon: BookOpen },
    ],
  },
  {
    titleKey: "activities",
    items: [
      { nameKey: "assignments", href: "/student/assignments", icon: FolderKanban },
    ],
  },
  {
    titleKey: "achievements",
    items: [
      { nameKey: "certificates", href: "/student/certificates", icon: Award },
    ],
  },
  {
    titleKey: "account",
    items: [
      { nameKey: "settings", href: "/student/settings", icon: Settings },
    ],
  },
];
...
export default function StudentSidebar({ mobileOpen = false, onClose = () => {}, collapsed = false, onToggleCollapse = () => {} }: { mobileOpen?: boolean, onClose?: () => void, collapsed?: boolean, onToggleCollapse?: () => void }) {
  ...
  useEffect(() => {
    ...
    NAV_KEYS.forEach((section) => {
      const hasActiveItem = section.items.some(
        (item) => pathname === item.href || pathname.startsWith(item.href + "/")
      );
      if (hasActiveItem) {
        initialState[section.titleKey] = true;
      }
    });
