...
import { getPrograms, createProgram } from "@/lib/api/admin";
import { toast } from "sonner";
import { Plus, BookOpen, Clock, ChevronRight, Layers, Search } from "lucide-react";
import Link from "next/link";
import { CreateProgramModal } from "@/components/admin/lms/CreateProgramModal";
...
function ProgramCard({ program, t }: { program: any, t: any }) {
      ...
      >
        ...
        <div style={{ padding: "22px 22px 20px", display: "flex", flexDirection: "column", flex: 1 }}>
          ...
          <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 14 }}>
            ...
            <StatusBadge status={program.status} t={t} />
          </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px", paddingTop: "20px", borderTop: "1px solid #EEF0EA", marginTop: "auto" }}>
            <div>
              <div style={{ fontSize: 10, color: "#8A9E8C", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 4 }}>{t("totalCourses")}</div>
            ...
            <div>
              <div style={{ fontSize: 10, color: "#8A9E8C", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 4 }}>{t("publishedCount")}</div>
             ...
             <div style={{ gridColumn: "1 / -1", display: "flex", alignItems: "center", justifyContent: "space-between", paddingTop: "16px", borderTop: "1px dashed #EEF0EA" }}>
               <div style={{ display: "flex", alignItems: "center", gap: 6, color: "#8A9E8C", fontSize: 13, fontWeight: 500 }}>
                 <Clock size={14} />
                 {program.duration || t("durationSelfPaced")}
               </div>
...
export default function AdminProgramsPage() {
  ...
  const createMut = useMutation({
    ...
    onError: (err: any) => toast.error(err.response?.data?.message || t("toast.createFailed")),
  });
      ...
      {isLoading ? (
        ...
        <div style={{
          ...
          <h3 style={{ fontFamily: "Georgia, serif", fontSize: 22, fontWeight: 700, color: "#1C2B1E", margin: "0 0 8px 0" }}>
            {search ? t("empty.notFoundTitle") : t("empty.createFirstTitle")}
          </h3>
          <p style={{ fontSize: 14, color: "#8A9E8C", margin: "0 0 24px 0", maxWidth: 360, marginInline: "auto" }}>
            {search ? t("empty.notFoundDesc") : t("empty.createFirstDesc")}
          </p>
          {!search && (
            <button
              onClick={() => setShowModal(true)}
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(300px, 1fr))", gap: 24 }}>
          {programs.map((p) => <ProgramCard key={p.id} program={p} t={t} />)}
        </div>
      )}
