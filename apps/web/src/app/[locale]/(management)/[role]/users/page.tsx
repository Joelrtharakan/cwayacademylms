...
function StatusBadge({ user, t }: { user: any, t: any }) {
  if (user.isBanned)
    return (
      <span
        style={{
...
export default function AdminUsersPage() {
  ...
  const banMut = useMutation({
    ...
    onSuccess: () => { toast.success(t("toast.userBanned")); qc.invalidateQueries({ queryKey: ["admin-users"] }); setConfirmState(null); },
    onError: (e: any) => toast.error(e?.response?.data?.message || t("toast.banFailed")),
  });

  const unbanMut = useMutation({
    mutationFn: (id: string) => unbanUser(id),
    onSuccess: () => { toast.success(t("toast.userUnbanned")); qc.invalidateQueries({ queryKey: ["admin-users"] }); setConfirmState(null); },
    onError: (e: any) => toast.error(e?.response?.data?.message || t("toast.unbanFailed")),
  });

  const deleteMut = useMutation({
    mutationFn: (id: string) => deleteUser(id),
    onSuccess: () => { toast.success(t("toast.userDeleted")); qc.invalidateQueries({ queryKey: ["admin-users"] }); setConfirmState(null); },
  ...
  const columns: Column<any>[] = [
    ...
    { key: "role", header: t("table.role"), sortable: true, render: (row) => <RoleBadge role={row.role} /> },
    { key: "church", header: t("table.church"), render: (row) => <span style={{ fontSize: "13px", fontWeight: 500, color: "#8F9E93" }}>{row.church || "—"}</span> },
    { key: "location", header: t("table.location"), render: (row) => <span style={{ fontSize: "13px", fontWeight: 500, color: "#8F9E93" }}>{row.location || "—"}</span> },
    { key: "status", header: t("table.status"), render: (row) => <StatusBadge user={row} t={t} /> },
    { key: "createdAt", header: t("table.joined"), sortable: true, render: (row) => <span style={{ fontSize: "13px", fontWeight: 500, color: "#8F9E93" }}>{formatDate(row.createdAt)}</span> },
  ];
  ...
  const actions = (row: any) => (
    <DropdownMenu.Root>
      ...
      <DropdownMenu.Portal>
        ...
        >
          {[
            { label: t("actions.viewProfile"), icon: <Eye size={14} />, onClick: () => router.push(`${basePath}/users/${row.id}`) },
            {
              label: row.isBanned ? t("actions.unbanUser") : t("actions.banUser"),
            ...
            },
            {
              ...
              onClick: async () => {
                ...
                } catch { toast.error(t("toast.impersonateFailed")); }
              },
            },
            { label: t("actions.deleteUser"), icon: <Trash2 size={14} />, onClick: () => setConfirmState({ type: "delete", user: row }), danger: true },
          ].map((item, i) => (
  ...
  return (
    <div>
      <PageHeader
        title={t("title")}
        subtitle={t("subtitle", { total: data?.total ?? 0 })}
        actions={
          <button
        ...
        {[
          {
            ...
            options: [["", t("allRoles")], ["ADMIN", t("roleAdmin")], ["INSTRUCTOR", t("roleInstructor")], ["STUDENT", t("roleStudent")]],
          },
          {
            value: statusFilter,
            setter: (v: string) => { setStatusFilter(v); setPage(1); },
            options: [["", t("allStatus")], ["active", t("statusActive")], ["banned", t("statusBanned")], ["unverified", t("statusUnverified")]],
          },
        ].map((sel, i) => (
          <select
            key={i}
      ...
      {confirmState?.type === "ban" && (
        <ConfirmDialog
          ...
          title={t("confirm.banTitle", { name: confirmState.user.name })}
          description={t("confirm.banDesc")}
          confirmLabel={t("confirm.banConfirm")}
          danger
      ...
      {confirmState?.type === "unban" && (
        <ConfirmDialog
          open={true}
          onOpenChange={() => setConfirmState(null)}
          title={t("confirm.unbanTitle", { name: confirmState.user.name })}
          description={t("confirm.unbanDesc")}
          confirmLabel={t("confirm.unbanConfirm")}
      ...
      {confirmState?.type === "delete" && (
        <ConfirmDialog
          open={true}
          onOpenChange={() => setConfirmState(null)}
          title={t("confirm.deleteTitle", { name: confirmState.user.name })}
