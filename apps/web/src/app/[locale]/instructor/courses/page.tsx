...
function StatusBadge({ status, t }: { status: string, t: any }) {
  ...
  return <span style={{ background: s.bg, color: s.text, borderRadius: 6, padding: "4px 10px", fontSize: 10, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.08em" }}>{t(`filters.${status}`)}</span>;
}
    ...
    >
      ...
      <div style={{ padding: "20px", display: "flex", flexDirection: "column", flex: 1 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "12px", fontSize: "13px", color: "#8F9E93", fontWeight: 500, marginBottom: "20px" }}>
          ...
          <span style={{ display: "flex", alignItems: "center", gap: "4px" }}><Users size={14} /> {t("studentsCount", { count: course._count?.enrollments || 0 })}</span>
        </div>
        {/* Footer */}
    ...
    <div style={{ maxWidth: "1400px" }}>
      ...
      {isLoading ? (
        ...
        <div style={{ textAlign: "center", padding: "80px 0", background: "#FFFFFF", border: "1px dashed #DCE0D5", borderRadius: "16px" }}>
          ...
          <h2 style={{ fontFamily: "Georgia, serif", fontSize: "22px", color: "#1A261D", marginBottom: "8px", fontWeight: 700 }}>{t("noCourses")}</h2>
          <p style={{ color: "#8F9E93", fontSize: "14px", fontStyle: "italic", marginBottom: "4px", fontWeight: 500 }}>{t("verseQuote")}</p>
          <p style={{ color: "#8F9E93", fontSize: "12px" }}>{t("verseRef")}</p>
        </div>
      ) : (
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(320px, 1fr))", gap: "24px" }}>
          {courses.map((c: any) => (
