...
function MessagesContent({ t }: { t: any }) {
  ...
  const sendMut = useMutation({
    ...
    onError: () => toast.error(t("toastSendFail")),
  });
    ...
    <div style={{ display: "flex", height: "calc(100vh - 60px - 48px)", gap: 0, background: SURFACE, borderRadius: 16, overflow: "hidden", border: "1px solid var(--border-light, #E2E8F0)", boxShadow: "var(--shadow-sm, 0 1px 3px rgba(0,0,0,0.05))" }}>
      ...
      <div style={{ width: 320, flexShrink: 0, borderRight: "1px solid var(--border-light, #E2E8F0)", display: "flex", flexDirection: "column", background: "var(--cream-light, #F9FAF8)" }}>
        <div style={{ padding: "20px 20px 16px" }}>
          <h2 style={{ fontFamily: "var(--font-cinzel), serif", fontSize: 20, fontWeight: 700, color: DARK, marginBottom: 16, letterSpacing: "0.5px" }}>{t("title")}</h2>
          <div style={{ display: "flex", alignItems: "center", gap: 8, background: "#FFFFFF", borderRadius: 8, padding: "10px 14px", border: "1px solid var(--border-light, #E2E8F0)" }}>
            ...
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder={t("searchPlaceholder")} style={{ background: "transparent", border: "none", outline: "none", color: DARK, fontSize: 14, flex: 1 }} />
          </div>
        </div>
        <div style={{ flex: 1, overflowY: "auto" }}>
          {filteredConvos.length === 0 ? (
            <div style={{ padding: 24, textAlign: "center", color: MUTED, fontSize: 14 }}>{t("emptyList")}</div>
          ) : (
      ...
      {selectedUser ? (
        <div style={{ flex: 1, display: "flex", flexDirection: "column" }}>
          ...
          <div style={{ padding: "16px 24px", borderTop: "1px solid var(--border-light, #E2E8F0)", display: "flex", gap: 12, alignItems: "flex-end", background: "#FFFFFF" }}>
            <textarea value={content} onChange={e => setContent(e.target.value)} onKeyDown={handleKeyDown} placeholder={t("typePlaceholder")} rows={1}
        ...
        <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column", gap: 12 }}>
          ...
          <p style={{ color: MUTED, fontSize: 14 }}>{t("selectPrompt")}</p>
        </div>
      )}
    </div>
  );
