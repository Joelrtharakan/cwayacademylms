...
export default function ThumbnailPromoSection({ course, onSave }: { course: any, onSave: () => void }) {
  ...
  const [localThumbPreview, setLocalThumbPreview] = useState<string | null>(null);
  const [localPromoPreview, setLocalPromoPreview] = useState<string | null>(null);
  ...
  const uploadPromoMut = useMutation({
    mutationFn: (file: File) => {
      setLocalPromoPreview(URL.createObjectURL(file));
      const formData = new FormData();
  ...
  return (
    <div style={{ display: "grid", gap: "32px" }}>
      ...
      <div style={{ background: "#FFFFFF", borderRadius: "16px", padding: "32px", boxShadow: "0 4px 12px #E4E8E0" }}>
        ...
        >
          {uploadThumbMut.isPending ? (
            <>
              {(localThumbPreview || thumbnailUrl) && (
                <img src={localThumbPreview || thumbnailUrl} alt="Thumbnail" style={{ position: "absolute", top: 0, left: 0, width: "100%", height: "100%", objectFit: "cover", opacity: 0.5 }} />
              )}
              <div style={{ position: "absolute", zIndex: 10, display: "flex", flexDirection: "column", alignItems: "center", background: "rgba(255,255,255,0.8)", padding: "12px 24px", borderRadius: "12px" }}>
                <Loader2 size={32} className="animate-spin" color="#1A261D" style={{ margin: "0 auto 8px auto" }} />
                <p style={{ fontSize: "14px", color: "#1A261D", fontWeight: 700, margin: 0 }}>Uploading...</p>
              </div>
            </>
          ) : localThumbPreview || thumbnailUrl ? (
            <img src={localThumbPreview || thumbnailUrl} alt="Thumbnail" style={{ position: "absolute", top: 0, left: 0, width: "100%", height: "100%", objectFit: "cover" }} />
          ) : (
            <>
      ...
      <div style={{ background: "#FFFFFF", borderRadius: "16px", padding: "32px", boxShadow: "0 4px 12px #E4E8E0" }}>
        ...
        >
          {uploadPromoMut.isPending ? (
            <>
              {(localPromoPreview || promoUrl) && (
                <video src={localPromoPreview || promoUrl} style={{ position: "absolute", top: 0, left: 0, width: "100%", height: "100%", objectFit: "cover", opacity: 0.5 }} />
              )}
              <div style={{ position: "absolute", zIndex: 10, display: "flex", flexDirection: "column", alignItems: "center", background: "rgba(255,255,255,0.8)", padding: "12px 24px", borderRadius: "12px" }}>
                <Loader2 size={32} className="animate-spin" color="#1A261D" style={{ margin: "0 auto 8px auto" }} />
                <p style={{ fontSize: "14px", color: "#1A261D", fontWeight: 700, margin: 0 }}>Uploading...</p>
            ...
            <video src={localPromoPreview || promoUrl} controls style={{ position: "absolute", top: 0, left: 0, width: "100%", height: "100%", objectFit: "cover" }} />
          ) : (
            <>
              <UploadCloud size={32} color="#A0AEC0" style={{ margin: "0 auto 12px auto" }} />
      ...
      <div style={{ display: "flex", justifyContent: "flex-end" }}>
        <button 
          onClick={() => updateMut.mutate({ thumbnail: thumbnailUrl, promoVideoUrl: promoUrl })} 
